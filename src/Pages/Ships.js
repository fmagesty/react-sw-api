...
const Ships = () => {
  ...
  const FunctionFetchNaves = async () => {
    ...
    let nomeNaves = responseBody.results.map((resultado, key) => (
      <button
        className="nave-btn"
        key={key}
        onClick={() => FunctionFetchNavesInfo(key)}
      >
        {resultado.name}
      </button>
    ));
  ...
  const FunctionFetchNavesInfo = async (props) => {
    ...
    return setInfo(
      Object.entries(result).map((items, key) => (
        <li className="nave-info" key={key}>
          {items[0] + " : " + items[1]}{" "}
        </li>
      ))
    );
  ...
  return (
    <div className="nav-bar">
      <button className="nave-search" onClick={() => FunctionFetchNaves(nave)}>
        Fetch ships
      </button>
      <p>{nave}</p>
      <ul className="nave-info-ul">{info}</ul>
    </div>
  );
};
